...
void FragTrap::attack(std::string const &target) const {
  std::cout << "\n[ Attack ] FragTrap " << getName() << " attacks " << target
...
void FragTrap::takeDamage(unsigned int amount) {
  ...
  _hitPoints -= amount;
  std::cout << "\n[ Take Damage ] FragTrap " << getName() << " got a " << amount
            << " damages " << std::endl;
...
void FragTrap::beRepaired(unsigned int amount) {
  unsigned int tmp = _hitPoints;
  _hitPoints += amount;
  std::cout << "\n[ Repaired ] FragTrap " << getName() << " recovered "
            << amount << " HP" << std::endl;
...
std::ostream &operator<<(std::ostream &c, const FragTrap &frag) {
  c << "<" << frag.getName() << "> HP: " << frag.getHP()
    << " MP: " << frag.getMP() << " AD: " << frag.getAD();
  return c;
}
